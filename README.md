...
→ 물론 있었다. 우선 반응형 페이지 구성을 위해 별도의 media 태그로 기존의 스타일을 재 정의할때 문제가 있었다. 바로 클래스의 구체성 문제였다. 기존에 선택자를 좀 더 구체적으로 스타일하고 media 태그내에서는 그보다는 덜 구체적으로 스타일링을 했더니 사이즈 변화에 맞게 일부 스타일링이 적용되지 않았었다. 그리고 모바일 대응 웹 페이지 구성을 위해서는 내부 컨텐츠들을 또 다른 wrapper container로 감싸서 처리해주는 것이 내부 컨텐츠 정렬에 있어 유용하다는 것을 배웠다.
